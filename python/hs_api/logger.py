...
def setup_logger(logger, level=logging.INFO):
    ...
    handler1 = logging.FileHandler("hs_api.log")
    logFormatter = logging.Formatter(
        "%(asctime)s.%(msecs)03d %(filename)-20s %(lineno)4d %(levelname)-10s  %(message)s")
    handler1.setFormatter(logFormatter)
...
logger = logging.getLogger('hs_benchmark')
